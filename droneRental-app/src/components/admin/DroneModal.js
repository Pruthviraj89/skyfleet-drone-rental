...
const DroneModal = ({ open, onClose, drone, onSave }) => {
  ...
  useEffect(() => {
    if (isEdit && drone) {
      ...
      setForm({ model: '', brand: '', status: 'available', pricePerHour: '', batteryLife: '', location: '', imageUrl: '', guideUrl: '' });
    }
  }, [drone, isEdit, open]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Basic validation
      if (!form.model || !form.brand || !form.status || !form.pricePerHour || !form.batteryLife || !form.location) {
      ...
      <div className="modal-dialog" role="document">
        <div className="modal-content">
          <div className="modal-header bg-success text-white">
            <h5 className="modal-title">{isEdit ? 'Edit Drone' : 'Add Drone'}</h5>
          ...
          <form onSubmit={handleSubmit}>
            <div className="modal-body">
              ...
              <div className="mb-3">
                ...
                <input type="number" className="form-control" name="pricePerHour" value={form.pricePerHour} onChange={handleChange} required min="0" step="0.01" />
              </div>
              <div className="mb-3">
                <label className="form-label">Battery Life (minutes)</label>
                <input type="number" className="form-control" name="batteryLife" value={form.batteryLife} onChange={handleChange} required min="0" />
              </div>
              <div className="mb-3">
              ...
              <div className="mb-3">
                ...
                <input type="text" className="form-control" name="guideUrl" value={form.guideUrl} onChange={handleChange} />
              </div>
            </div>
